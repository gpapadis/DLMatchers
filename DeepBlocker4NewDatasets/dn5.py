...
left_df = pd.read_csv(main_dir + 'D6Aemb.csv', sep=deli)
left_df = left_df[['id', 'Aggregate Value']]
right_df = pd.read_csv(main_dir + 'D6Bemb.csv', sep=deli)
right_df = right_df[['id', 'Aggregate Value']]
golden_df = pd.read_csv(main_dir + 'D6groundtruth.csv', sep=deli)
...
candidate_set_df.to_csv(main_dir + "D6candidates.csv", sep=deli, index=False)
print(results)