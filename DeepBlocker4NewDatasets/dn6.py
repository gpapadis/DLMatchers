...
left_df = pd.read_csv(main_dir + "D7Bemb.csv", sep=deli)
left_df = left_df[['id', 'Clean Ag.Value']]
right_df = pd.read_csv(main_dir + "D7Aemb.csv", sep=deli)
right_df = right_df[['id', 'Clean Ag.Value']]
golden_df = pd.read_csv(main_dir + "D7groundtruthRev.csv", sep=deli)
...
candidate_set_df.to_csv(main_dir + "D7candidates.csv", sep=deli, index=False)
print(results)