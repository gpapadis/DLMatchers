...
left_df = pd.read_csv(main_dir + 'D3Aemb.csv', sep=deli)
left_df = left_df[['id', 'Title']]
right_df = pd.read_csv(main_dir + 'D3Bemb.csv', sep=deli)
right_df = right_df[['id', 'Title']]
golden_df = pd.read_csv(main_dir + 'D3groundtruth.csv', sep=deli)
cols_to_block = ['Title']
...
candidate_set_df.to_csv(main_dir + "D3candidates.csv", sep=deli, index=False)
print(results)